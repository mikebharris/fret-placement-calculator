...
func (s *steps) iAmProvidedWithTheFretPlacementsForJustIntonation() error {
	...
	fretting := Fretting{}
	if err := json.Unmarshal([]byte(responseFromLambda.Body), &fretting); err != nil {
		return fmt.Errorf("unmarshalling result: %s", err)
	}

	assert.Equal(s.t, "ji", fretting.System)
	assert.Equal(s.t, 540.0, fretting.ScaleLength)
	assert.Equal(s.t, 15, len(fretting.Frets))

	assert.Equal(s.t, "16:15", fretting.Frets[0].Label)
	assert.Equal(s.t, 33.75, fretting.Frets[0].Position)

	assert.Equal(s.t, "2:1", fretting.Frets[14].Label)
	assert.Equal(s.t, 270.0, fretting.Frets[14].Position)
	return nil
}
